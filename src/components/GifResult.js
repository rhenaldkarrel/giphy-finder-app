...
const getData = async () => {
	try {
		const url = `http://api.giphy.com/v1/gifs/search?q=mario&api_key=${process.env.REACT_APP_GIPHY_KEY}&limit=12`;
...
const data = getData().then((gifs) => {
	console.log(gifs);
	const data = gifs.forEach((gif) => (
		<figure key={gif.id}>
			<img src={gif.url} alt='Result' />
